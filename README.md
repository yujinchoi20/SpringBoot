...
* 클라이언트가 요청한 정보(데이터)를 1차 캐시 혹은 DB에서 조회하여 __Model__ 객체에 담아 __View__ 화면으로 보낸다. 
* __View__ 화면은 __Model__ 객체에 있는 정보(데이터)를 사용하여 클라이언트에게 보여줄 화면을 구성한다. 
* 이때 __Model__ 객체에 데이터를 담은 주체를 __Controller__ 라고 한다. 


#### __변경 감지 기능__

* 정보 수정은 기존 정보를 조회(select)하여 수정(setter)하고 다시 저장(update)하는 방식으로 진행한다. 
* 만약 정보를 수정하지 않았는데 update 쿼리문을 실행하면 의미가 있을까? 없다. 그래서 정보 수정이 없으면 update를 진행하지 않고, 정보 수정이 있을 때만 update를 진행하는 변경 감지 기능을 사용한다.
* 변경 감지 기능은 영속화 되어 있는 객체에 대해서만 동작한다. 준영속화 되어 있는 객체를 대상으로 변경 감지 기능을 적용하려하면 일반적인 update 쿼리문을 실행하는 것과 똑같다.
* 때문에 객체가 영속화 되어 있는 시점에서 변경 감지 기능을 사용해야 한다.
* 비즈니스 로직에 update 메서드를 만들어 놓고 Controller 계층에서 호출해 사용하였다.
...
* css와 js는 Bootstrap에서 다운받아 사용하였다. 
* jumbotron-narrow.css 파일을 추가로 작성하여 뷰 화면을 정렬하였다.
